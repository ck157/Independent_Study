...
selected_options <- sort(BS_Delta-BSGP_Delta, index.return=TRUE, decreasing=TRUE)$ix[1:5]
...
# grid.newpage()
#Spot vs Strike
ggplot(mapping=aes(x=stan_dat$x[,1],y=stan_dat$x[,2], color = IC)) +
  geom_point(size = 2) +
...
ggplot(mapping=aes(x=stan_dat$x[,1],y=stan_dat$x[,3], color = IC)) +
  geom_point(size = 2) +
...
ggplot(mapping=aes(x=stan_dat$x[,1],y=stan_dat$x[,4], color = IC)) +
  geom_point(size = 2) +
...
ggplot(mapping=aes(x=stan_dat$x[,1],y=stan_dat$x[,5], color = IC)) +
  geom_point(size = 2) +
...
ggplot(mapping=aes(x=stan_dat$x[,1],y=stan_dat$x[,6], color = IC)) +
  geom_point(size = 2) +
...
ggplot(mapping=aes(x=stan_dat$x[,1],y=stan_dat$y, color = IC)) +
  geom_point(size = 2) +
...
##Don't make portfolio for Delta  Hedging strategy, instead choose where the delta is most different(maybe top quantile for highest differences-HOW TO CHOOSE? WHAT LOGIC TO USE FOR CHOOSING THIS?) then use them only. Using all of them will only reduce the effect.

##Evaluate based on payoff of the delta hedge rather than comparing with the realized delta
