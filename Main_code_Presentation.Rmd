...
9-1 Generating BSGP prices for Backtesting
...
x2_backtest <- cbind(x_1_backtest,x_2_backtest,x_3_backtest,x_4_backtest,x_5_backtest,x_6_backtest)
```

```{r computing blackscholes for backtest}
x2_bs_backtest <- cbind(as.numeric(stan_dat$total_puts_American$forward_price[test_start_backtest:test_end_backtest]),as.numeric(stan_dat$total_puts_American$strike_price[test_start_backtest:test_end_backtest]),as.numeric(stan_dat$total_puts_American$impl_volatility[test_start_backtest:test_end_backtest]),as.numeric(stan_dat$total_puts_American$time_to_exp[test_start_backtest:test_end_backtest]),x.grid_5 <- as.numeric(stan_dat$total_puts_American$dividend_yield[test_start_backtest:test_end_backtest]),as.numeric(stan_dat$total_puts_American$interest_rate[test_start_backtest:test_end_backtest]))
...
for (row in 1:nrow(data.frame(x2_bs_backtest))){
  blackscholes_test_backtest[row] <- as.numeric(blackscholes(-1,S0=as.numeric(stan_dat$total_puts_American$forward_price[test_start_backtest:test_end_backtest])[row],K=as.numeric(stan_dat$total_puts_American$strike_price[test_start_backtest:test_end_backtest])[row],r=as.numeric(stan_dat$total_puts_American$interest_rate[test_start_backtest:test_end_backtest])[row],t=as.numeric(stan_dat$total_puts_American$time_to_exp[test_start_backtest:test_end_backtest])[row],vola=as.numeric(stan_dat$total_puts_American$impl_volatility[test_start_backtest:test_end_backtest])[row],divrate=as.numeric(stan_dat$total_puts_American$dividend_yield[test_start_backtest:test_end_backtest])[row])$Price)
}
...
ggplot(mapping=aes(x=log(y_mean_values_bs_backtest),y=log(stan_dat$total_puts_American$mid_price[test_start_backtest:test_end_backtest]))) +
  geom_point() +
  labs(title = "Predicted vs Data - Black-Scholes Integrated SGP",
       x = "Predicted Option Price",
       y = "Observed Option Price") +
...
MSE(y_mean_values_bs_backtest,stan_dat$total_puts_American$mid_price[test_start_backtest:test_end_backtest])
```

9-2 Computing BS Delta

```{r compute BSGP delta}
library(derivmkts)
BS_Delta <- greeks(bsput(s=as.numeric(stan_dat$total_puts_American$forward_price[test_start_backtest:test_end_backtest]), k=as.numeric(stan_dat$total_puts_American$strike_price[test_start_backtest:test_end_backtest]), v=as.numeric(stan_dat$total_puts_American$impl_volatility[test_start_backtest:test_end_backtest]), r=as.numeric(stan_dat$total_puts_American$interest_rate[test_start_backtest:test_end_backtest]), tt=as.numeric(stan_dat$total_puts_American$time_to_exp[test_start_backtest:test_end_backtest]), d=as.numeric(stan_dat$total_puts_American$dividend_yield[test_start_backtest:test_end_backtest])), complete=TRUE, long=FALSE, initcaps=TRUE)$Delta

BS_Delta
```

9-3 Computing BSGP vector C & vector K'
...
vector_k <- matrix(vector_k_values, 71, 322, byrow=FALSE)

BSGP_Delta <- BS_Delta + vector_k %*% vector_c_values
hist(abs(vector_c_values))
```

9-4 Computing Actual Delta for the next day

```{r}
tmr_start_backtest <- 394 #6/11
tmr_end_backtest <- 564 #6/11

tmr_option_price <- stan_dat$total_puts_American$mid_price[tmr_start_backtest:tmr_end_backtest]
today_option_price <- stan_dat$total_puts_American$mid_price[test_start_backtest:test_end_backtest]

tmr_underlying_price <- stan_dat$total_puts_American$forward_price[tmr_start_backtest:tmr_end_backtest]
today_underlying_price <- stan_dat$total_puts_American$forward_price[test_start_backtest:test_end_backtest]

library("data.table")
library("fastmatch")

tmr_data <- setDT(data.frame(cbind(stan_dat$total_puts_American$strike_price[tmr_start_backtest:tmr_end_backtest],stan_dat$total_puts_American$exdate[tmr_start_backtest:tmr_end_backtest])))
today_data <- setDT(data.frame(cbind(stan_dat$total_puts_American$strike_price[test_start_backtest:test_end_backtest],stan_dat$total_puts_American$exdate[test_start_backtest:test_end_backtest])))

compare_rows <- (tmr_data$X1 %fin% today_data$X1) & (tmr_data$X2 %fin% today_data$X2)

tmr_valid_rows <- tmr_data[which(compare_rows == TRUE)]
today_valid_rows <- today_data[which(compare_rows == TRUE)]

```



##Improve JBES 3.3 (Delta part)
##Interpret Ci and color plot (ref ipad)
##Backtest delta neutral hedging strategy
