...
for (number in IC){
  ...
  else if (number < unname(quantile(IC)[3])){
    Low_DAD <- append(Low_DAD, number)
  }
  else if (number < unname(quantile(IC)[4])){
    High_DAD <- append(High_DAD, number)
  }
  else {
...
prune.DAD <- prune.misclass(tree.DAD, best = 5)
plot(prune.DAD)
...
##What does the DAD really mean in financial aspect -> the idea of sensitivity of the training data points to the model means WHAT???

##Complete Section 4 with DAD analysis - classification model (TREES(boosting,rf,bagging) aand then generate importance plots or interpret results, multinomial model and fit with response of 0,1,2,3 of DAD quantiles) (https://stats.idre.ucla.edu/r/dae/ordinal-logistic-regression/) maybe use AIC,BIC for model selection
