...
fig_bs_sd <- fig_bs_sd %>% layout(title = 'BSGP', 
  scene = list(xaxis = list(title='Spot Price'), yaxis = list(title='Strike Price'), zaxis= list(title='Standard Deviation', range = c(min(0,max(y_sd_values_cont_bs)))),camera = list(
    eye=list(x=-1.25,y=-1.25,z=1.25)
  )
  )
)
