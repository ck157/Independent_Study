...
plot_gp_realizations <- function(pred_gp, post_data, title) {
  params <- extract(pred_gp)
  I <- length(params$y_predict[,1])
  
  c_superfine <- c("#8F272705")
  plot(1, type="n", xlab="x", ylab="y", main=title,
       xlim=c(min(post_data$x1), max(post_data$x1)), ylim=c(min(post_data$y1), max(post_data$y1)))
  for (i in 1:I)
    lines(post_data$x2, params$y_predict[i,],col=c_superfine)
  
  points(post_data$x1, post_data$y1, col="black", pch=16, cex=0.6)
  points(post_data$x2, colMeans(params$y_predict), col="red", pch=16, cex=0.4)
  library(matrixStats)
  probs <- c(0.25, 0.5, 0.975)
  q <-colQuantiles(params$y_predict,probs=probs)
  points(post_data$x2,q[,1], col="yellow", pch=16, cex=0.3)
  points(post_data$x2,q[,3], col="yellow", pch=16, cex=0.3)
}
