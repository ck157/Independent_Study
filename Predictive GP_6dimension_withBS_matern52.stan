    ...
    
    {
      for (i in 1:N1) {
        ...
        for (j in (i+1):N1) { //off-diagonal
          ...
          for (l in 1:variables){
            K[i, j] = K[i,j]*(1+(sqrt(5*square(x_test[i,l]-x_test[j,l]))/theta[l])+(5*square(x_test[i,l]-x_test[j,l])/3*square(theta[l]))) * (exp(-sqrt(5*square(x_test[i,l]-x_test[j,l]))/theta[l]));
      ...
      for (i in 1:N1) {
        for (j in 1:N2) { //off-diagonals
          k_x1_x2[i,j] = 1.0;
          for (l in 1:variables){
            k_x1_x2[i, j] = k_x1_x2[i,j]*(1+(sqrt(5*square(x_test[i,l]-x_test[j,l]))/theta[l])+(5*square(x_test[i,l]-x_test[j,l])/3*square(theta[l]))) * (exp(-sqrt(5*square(x_test[i,l]-x_test[j,l]))/theta[l]));
            // K[i, j] = gamma2*exp(-dot_product(theta,square(x[i,] - x[j,])));
          }
        }  
      }   
      ...
      for (i in 1:N2) {
        ...
        for (j in (i+1):N2) { //off-diagonals
          K2[i,j] = 1.0;
          for (l in 1:variables){
            K2[i, j] = K2[i,j]*(1+(sqrt(5*square(x_test[i,l]-x_test[j,l]))/theta[l])+(5*square(x_test[i,l]-x_test[j,l])/3*square(theta[l]))) * (exp(-sqrt(5*square(x_test[i,l]-x_test[j,l]))/theta[l]));
            K2[j, i] = K2[i, j];  
          }
        }  
      }   
