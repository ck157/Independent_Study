    ...
    
    {
      ...
      for (i in 1:N1) {
        for (j in 1:N2) { //off-diagonals
          k_x1_x2[i,j]=gamma2*theta[1]*-2*(x_train[i,1]-x_test[j,1]);
          for (l in 1:variables){
      ...
      for (i in 1:N2) {
        ...
        for (j in (i+1):N2) { //off-diagonal
        K2[i, j] = 1;
          for (l in 1:variables){
            K2[i, j] = K2[i,j]*exp(-theta[l]*square(x_test[i,l] - x_test[j,l]));
          }
        K2[i,j] = 2*gamma2*theta[1]*(-2*K2[i,j]*theta[1]*square(x_test[i,1]-x_test[j,1])+K2[i,j]);
        K2[j, i] = K2[i, j];
        }  
      }   
